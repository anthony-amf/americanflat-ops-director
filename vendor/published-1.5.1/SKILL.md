...
- **Stedi**: the key is read from the environment, never embedded. If unset,
  `validate_stedi.py` loads it from `~/.yusen/yusen.env` (chmod 600, gitignored);
  `export STEDI_API_KEY=<key>` still overrides. See `references/stedi-api.md`.
...
- Add `--write` to persist the outcome back onto the invoice row (sets
  ...
  dispute specs, and human notes are preserved). This is how an invoice gets
...
Once you have those lines, hand them straight back to the validator with
`--detail` instead of writing the verdict by hand. It takes `QTY x RATE [label]`
items separated by `;`, checks the sum against the billed total, and stamps the
row:

```bash
python3 scripts/validate_rate_card.py 756442 \
  --detail "5383 x 4.47 storage; 220 x 4.47 storage bulk; 4 x 0.7312 per bin" --write
```

Every family here bills as a sum of qty × rate, so this closes Storage, VAS,
Receiving and Admin — and SP/LTL's rate axis, though SP/LTL still needs the
Step 3 Stedi gate before payment.

Two things to be careful about:

- **Give the rate as billed, not the card rate.** US storage bills below card
  (above), so multiplying a pallet count by the card rate would manufacture a
  ~25% discrepancy on every storage invoice. The validator compares the supplied
  rate to the card separately and labels below-card as a stale card, not a dispute.
- **A `--detail` verdict is sticky.** It records `validated_by` as
  `yusen-invoice-validator (detail supplied by operator)`, which the human-stamp
  rule (Step 5) treats as a person's verdict — so the next scheduled header sweep
  refreshes the timestamp but leaves the status alone instead of reverting it to
  `needs_detail`. A later `discrepancy` or `disputed` still overrides it.
