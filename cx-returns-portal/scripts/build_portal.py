...
def build(routing: dict) -> str:
    payload = json.dumps(routing, indent=2)
    # The template carries literal braces (CSS, JS), so substitute rather than format.
    return TEMPLATE.replace("__ROUTING_JSON__", payload)
...
<div class="wrap">
  <header class="masthead">
    ...
    <p>Paste what the customer sent. Pick the case. Copy the warehouse email.
...
const state = {
  ...
  disposition: 'restock', sender: '', title: 'Customer Experience',
  hasReplacement: false, touched: new Set(),
};
...
function missingFields(wh) {
  ...
  for (const need of (spec ? spec.needs : [])) {
...
$('reset').addEventListener('click', () => {
  ...
  Object.assign(state, { caseType:'', warehouse:'', order:'', rs:'', tracking:'', carrier:'',
    marketplace:'', customer:'', skus:'', deadline:'EOD today', disposition:'restock',
