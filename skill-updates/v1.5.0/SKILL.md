...
**Hourly labor is checked per warehouse (v1.5.0).** The MSA prices the same role
differently by site, so an hourly rate is only checked against the schedule for
the warehouse that billed it (`MSA_HOURLY_RATES`). South Carolina physical
inventory and stock consolidation are **$63.00/hr**; SC general labor is $53.55,
NJ is $53.55, Fontana is $59.8278. A line billed at another site's labor rate —
$63.00 on an NJ or Fontana invoice, say — is reported as an **off-card labor
rate** and held at `needs_detail` with the site's own hourly schedule printed in
the report card, instead of passing as an ad-hoc VAS job rate. That is a rebill
request, not an MSA dispute, so it does **not** stamp `disputed`. Warehouses that
don't normalize fall back to accepting every site's rates, so an unmapped row is
never flagged on a guess.
