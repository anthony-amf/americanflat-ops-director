...
# =====================================================================
# MSA line-level engine (v1.5.0)
#
# Rate tables from the AMERICANFLAT Yusen MSA (draft 7.15.2026; rates confirmed
# final by Anthony 8/5/26; validity April 2026 – March 31 2027). Extracted from
# the embedded rate image (word/media/image2.emf). Update at contract renewal.
# The Notion card mirrors these; these tables exist so the unattended sweep can
# verify lines without a live Notion fetch. Per-unit rates are site-independent
# (the label carries the site); hourly labor is scoped per warehouse below.
# =====================================================================
...
def _fmt_rate(rate: float) -> str:
    """$63.00 / $77.70 / $59.8278 — full precision, never fewer than 2dp."""
    whole, _, frac = f"{rate:,.4f}".rstrip("0").partition(".")
    return "$" + whole + "." + (frac + "00")[:max(2, len(frac))]


def _rate_match_priority(rate: float, full: float):
    """How a printed rate could stand for a full-precision one, or None.
    ...
    if abs(rate - full) <= 0.0006:
        return 0                                            # printed in full
    if abs(_math.floor(full * 100) / 100 - rate) <= 0.0011:
        return 1                                            # truncated to 2dp
    if abs(round(full, 2) - rate) <= 0.0011:
        return 2                                            # rounded to 2dp
    return None


def _hourly_table(wh: Optional[str]) -> dict:
    """Hourly rates on schedule at one site: that site's roles + national lines.

    An unknown/unmapped warehouse falls back to every site's rates — the
    pre-1.5.0 behaviour — so a row whose warehouse text doesn't normalize is
    never flagged off-card on a guess."""
    if wh in MSA_HOURLY_RATES:
        return {**MSA_HOURLY_RATES_ALL, **MSA_HOURLY_RATES[wh]}
    merged = dict(MSA_HOURLY_RATES_ALL)
    for site in MSA_HOURLY_RATES.values():
        merged.update(site)
    return merged


def _off_site_hourly_table(wh: Optional[str]) -> dict:
    """Hourly rates on ANOTHER site's schedule but not on this one's.

    Billing one DC at another DC's labor rate is the mis-keyed-rate signature
    (SC's $63 physical-inventory rate on an NJ work order, say). Those lines are
    parsed and named rather than silently accepted as an ad-hoc job rate."""
    if wh not in MSA_HOURLY_RATES:
        return {}
    here = _hourly_table(wh)
    elsewhere = {}
    for site, table in MSA_HOURLY_RATES.items():
        if site == wh:
            continue
        for full, role in table.items():
            if any(abs(full - r) <= 0.0011 for r in here):
                continue
            elsewhere.setdefault(full, (role, []))[1].append(SITE_LABEL[site])
    return {full: f"off-card hourly ({_fmt_rate(full)} is the {role} rate at {' / '.join(sites)})"
            for full, (role, sites) in elsewhere.items()}


def _rate_candidates(rate: float, wh: Optional[str] = None) -> list:
    """All (full_rate, label, flag, priority) an invoice-printed rate could mean.

    `flag` is None for a clean MSA line, "disputed" for a known MSA-conflict
    rate, or "off_card" for another site's hourly rate. Hourly tables are
    warehouse-scoped; everything else is site-independent."""
    out = []
    tables = ((DISPUTED_LINE_RATES, "disputed"),
              (MSA_LINE_RATES, None),
              ({r: f"hourly {role}" for r, role in _hourly_table(wh).items()}, None),
              (_off_site_hourly_table(wh), "off_card"))
    for table, flag in tables:
        for full, label in table.items():
            prio = _rate_match_priority(rate, full)
            if prio is not None:
                out.append((full, label, flag, prio))
...
def _parse_charge_lines(text: str, amount: float, invoice_type: str,
                        wh: Optional[str] = None) -> list:
    """Parse charge lines and verify each against the MSA table.

    Returns [{qty, rate, amt, label, disputed, off_card}] — only lines whose
    math verifies at a known rate. Page-1 + worksheet echoes are deduped.
    `wh` is the normalized warehouse key; hourly rates are checked against that
    site's MSA schedule."""
    ...
    for pat in _LINE_PATTERNS:
        for m in pat.finditer(text):
            ...
            if (qty, rate, la) in seen or qty <= 0 or rate <= 0 or la > amount + 0.05:
                continue
            cands = _rate_candidates(rate, wh)
            if not cands and _rate_candidates(qty, wh):
                qty, rate, cands = rate, qty, _rate_candidates(qty, wh)
            hit = None
            for full, label, flag, _prio in cands:
                if abs(qty * full - la) <= max(0.02, qty * 0.0006):
                    hit = (full, label, flag)
                    break
            if hit is None and cands and abs(qty * rate - la) <= 0.02:
                full, label, flag, _p = cands[0]
                hit = (full, label, flag)
            if hit is None and t == "vas" and rate <= 500 and \
                ...
                # An off-card hourly rate never lands here — it matched above.
                hit = (rate, f"job rate {rate}", None)
            if hit is None:
                continue
            full, label, flag = hit
            seen.add((qty, rate, la))
            if abs(full - 0.92) < 0.001 and t.startswith("sml"):
                label, flag = "pack-out (AF-7 removed)", "disputed"
            found.append(dict(qty=qty, rate=full, amt=la, label=label,
                              disputed=(flag == "disputed"),
                              off_card=(flag == "off_card")))
    best = {}
    for l in found:
        best[(l["qty"], l["amt"])] = l
...
def apply_line_pass(invoice: dict, result: dict, pdf_dir: Path) -> None:
    ...
    lines = _parse_charge_lines(text, amount, t, wh)
    if not lines and is_vas:
        ...
        for m in re.finditer(r"([\d,]+)\s*(?:[A-Z][^\n@$]{0,60})?@\s*\$?\s*([\d.]+)", text):
            ...
            if qty > 0 and 0 < rate <= 500 and abs(qty * rate - amount) <= max(0.02, qty * 0.01):
                # Same off-card check as the parsed path: an OCR'd line at
                # another site's labor rate is named, not waved through.
                off = next((lab for full, lab, flag, _p in _rate_candidates(rate, wh)
                            if flag == "off_card"
                            and abs(qty * full - amount) <= max(0.02, qty * 0.01)), None)
                lines = [dict(qty=qty, rate=rate, amt=amount, disputed=False,
                              off_card=bool(off),
                              label=(f"{off} (OCR, reconciles to total)" if off
                                     else f"job rate {rate} (OCR, reconciles to total)"))]
                break
    disputed = [l for l in lines if l["disputed"]]
    off_card = [l for l in lines if l.get("off_card")]
    ...
    if disputed:
        ...
        result["line_report"] = (
            ...
            + (f" Also {len(off_card)} off-card labor line(s): {_line_spec(off_card)}." if off_card else "")
            + (" Stedi order-level pass still pending (SP/LTL gate)." if is_spltl else ""))
    elif off_card:
        oc_amt = round(sum(l["amt"] for l in off_card), 2)
        site = SITE_LABEL.get(wh, wh or "this warehouse")
        schedule = ", ".join(f"{_fmt_rate(r)} {role}"
                             for r, role in sorted(_hourly_table(wh).items()))
        result["status"] = "needs_detail"
        result["line_report"] = (
            f"OFF-CARD LABOR RATE for {site}: {_line_spec(off_card)} => ${oc_amt:,.2f} billed "
            f"at an hourly rate that is not on {site}'s MSA schedule ({site} hourly: {schedule}). "
            "Check the work order for the work type before paying — a wrong-site rate is a "
            "rebill request, not an MSA dispute."
            + (f" Remaining lines at MSA-schedule rates cover ${explained - oc_amt:,.2f}."
               if explained - oc_amt > 0.005 else "")
            + ("" if cover else f" Unexplained ${residual:,.2f}."))
    elif is_vas:
