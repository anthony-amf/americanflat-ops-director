...
Validation depth:
  - Header pass (always): Admin weekly fee (pro-rated), intl breakdown sum-check.
  - Line-level pass (v1.2.0, US invoices): downloads the invoice PDF (cache →
    Drive API via ADC → public link), parses the charge lines, and verifies
    qty x full-precision MSA rate = line amount. Yusen prints page-1 rates
    TRUNCATED to 2dp (1.7871 -> "1.78", 4.347 -> "4.34") while amounts use full
    precision — matching maps a printed rate to its candidate full-precision
    rates and lets the line math pick the right one.
  - Disputed detection: known MSA-conflict charges (AF-9 stretchwrap beside a
    $10 all-in pallet, AF-7 pack-out, 14.317 combined pallet) stamp `disputed`
    with the disputed $ in validation_variance. A `disputed` stamp is never
    downgraded by a re-sweep.
  - VAS documentation policy (Anthony, 2026-08-05): verified line math plus
    ...
    when there is no documentation for the amount or the validation errored.
  - SP/LTL is never stamped `valid` by this script alone — the Stedi
    order-level gate still applies (header pass notes are recorded).
...
def apply_line_pass(invoice: dict, result: dict, pdf_dir: Path) -> None:
    ...
    if existing in ("valid", "disputed"):
        result["_settled"] = existing
        return
...
def _merge_report(existing: Optional[str], block: str) -> str:
    """Replace any prior [AUTO-SWEEP ...] block with the fresh one, preserving
    manual/dispute/payment report text. Keeps daily sweeps from bloating the
    report card."""
    kept = re.sub(r"\n{0,2}\[AUTO-SWEEP.*?(?=\n\n\[|\Z)", "", existing or "", flags=re.S).strip()
    return (kept + ("\n\n" if kept else "") + block).strip()
...
def write_result(r: dict, client: bigquery.Client, invoice: Optional[dict] = None) -> tuple[bool, str]:
    """Persist the validation outcome back onto the invoice row.

    Records validated_at/status/variance and (v1.2.0) refreshes the [AUTO-SWEEP]
    section of validation_report on every write. Protection rules keep an
    unattended sweep from undoing deeper judgments:
      * a `disputed` stamp is never downgraded — it clears only when the
        invoice is re-billed/credited or manually cleared;
      * an existing `valid` stamp is never downgraded to needs_detail/error by
        a shallower pass (a `discrepancy` finding still overwrites — that is a
        real alarm);
      * rows the line pass marked settled are skipped entirely.
    ...
    """
    inv = invoice or {}
    old = (inv.get("validation_status") or "").strip()
    if r.get("_settled"):
        return True, f"kept {r['_settled']} (settled row — not re-stamped)"
    if old == "disputed" and r["status"] != "disputed":
        return True, "kept disputed (never downgraded by a re-sweep)"
    if old == "valid" and r["status"] in ("needs_detail", "error"):
        return True, "kept valid (deep-pass stamp not downgraded by a shallower pass)"

    block = (f"[AUTO-SWEEP {date.today().isoformat()}] {r['status'].upper()}"
             + (f" ${r['variance']:,.2f} disputed" if r["status"] == "disputed" and r.get("variance") else "")
             + ". " + (r.get("line_report") or "; ".join(r.get("discrepancies") or []) or "header pass")).strip()
    report = _merge_report(inv.get("validation_report"), block)
    ...
    WHERE invoice_number = @inv
      AND (@status = 'disputed' OR IFNULL(validation_status, '') != 'disputed')
    """
...
def fetch_invoice(invoice_number: str, client: bigquery.Client) -> Optional[dict]:
    ...
    # Progressively degrade for tables not yet provisioned via --init: full
    # tracking columns -> payment column only -> base columns.
    for extra in (", pdf_url, paid_at, validation_status, validation_report",
                  ", pdf_url, paid_at", ", pdf_url", ""):
        try:
            rows = list(client.query(base.format(extra=extra), job_config=cfg).result())
            return dict(rows[0]) if rows else None
        except Exception as e:
            if "Unrecognized name" not in str(e):
                raise
    return None
...
def print_report(r: dict) -> None:
    ...
    if r.get("_settled"):
        print(f"\n  → settled: already stamped {r['_settled']} in BigQuery — not re-judged")
    print(f"{'='*78}")
...
def _print_rollup(results: list) -> None:
    by = {"valid": 0, "discrepancy": 0, "needs_detail": 0, "error": 0, "disputed": 0}
    for r in results:
        key = r.get("_settled") or r["status"]
        by[key] = by.get(key, 0) + 1
    print(f"\n{'='*78}\nSUMMARY: {len(results)} invoices")
    print(f"  ✅ valid {by['valid']}   ⚠️ disputed {by['disputed']}   "
          f"🚨 discrepancy {by['discrepancy']}   "
