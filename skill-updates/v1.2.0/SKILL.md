...
- Add `--write` to persist the outcome back onto the invoice row (sets
  ...
  This is how an invoice gets "checked off" — a dashboard or query can then show
  what's been validated. A plain run never mutates the table; only `--write` does.
...
- `valid` ✅ — billed amount matches expected (within 1¢) / all charge lines
  verified at MSA-schedule rates.
- `disputed` ⚠️ — validated, but contains known MSA-conflict charges (AF-9
  stretchwrap/14.317 pallet+wrap, AF-7 pack-out). The disputed $ goes in
  `validation_variance`; short-pay/hold. **Never re-stamp a disputed row back
  to needs_detail** — it clears only on re-bill/credit or manual clearing
  (the script enforces this on `--write`).
- `discrepancy` 🚨 — real variance. The line explains billed vs expected.
- `needs_detail` ⏳ — awaiting deeper evidence: SP/LTL pending its Stedi
  order-level pass, a VAS PDF with no supporting documentation, a scanned PDF
  that couldn't be OCR'd, or unexplained residual dollars.
- `error` ❌ — unknown warehouse or no rate on contract.

How each type is checked (v1.2.0 — header pass + PDF line-level pass):
- **Line-level pass (all US types)** — the script fetches the invoice PDF
  (cache `~/.yusen-pdf-cache` → Drive API via ADC `drive.readonly` → public
  link), extracts text (OCR fallback for scanned SC VAS), parses charge lines
  and verifies qty × full-precision MSA rate = line amount. Page-1 rates
  print **truncated** to 2dp (1.7871 → "1.78", 4.347 → "4.34"); the matcher
  maps a printed rate to its candidate full-precision rates and the line math
  picks the winner. `--no-pdf` skips this; `--pdf-dir` moves the cache.
...
- **Storage / Receiving** → full line verification from the PDF → `valid`.
- **VAS** → documentation policy (Anthony, 2026-08-05): verified line math +
  supporting documentation pages in the PDF → `valid`; `needs_detail` only
  with no documentation for the amount, or on error.
- **Small Parcel / LTL** → line pass records an "MSA header pass complete"
  note but stays `needs_detail` until the Stedi order-level pass runs — the
  script never stamps SP/LTL `valid` on its own.
- Rows already `valid`/`disputed` in BigQuery are **settled**: the sweep skips
  the PDF fetch and never re-stamps them. Every `--write` refreshes a dated
  `[AUTO-SWEEP]` section in `validation_report` (replaced in place, other
  report text preserved).
...
- **Approval stores the report card.** `--mark-paid` writes the two-axis report
  to `validation_report` on the row — the audit artifact of what was checked at
  approval time. Auto-composed by default; when you performed a deeper
  document-level validation in-conversation (PDF worksheet qty×rate, NL
